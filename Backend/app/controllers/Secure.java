...
/**
 * Created by besteban on 7/11/14.
 */
public class Secure extends Controller {
    static int ServiceProcessCode = 1;
    ...
    public static void authenticate(@Required String username, @Required String password, boolean remember) throws Throwable {
        ...
        if(!allowed.equals("true")) {
            flash.keep("url");
            if (allowed.equals("blocked"))
                flash.error(Messages.get("controller.secure.user_blocked"));
            else
                flash.error(Messages.get("controller.secure.login_failure"));
            params.flash();
            login();
        }
        ...
        redirect(Router.reverse("Application.index").url);
    ...
    public static class Security extends Controller {
        ...
        static String authenticate(String username, String password) {
            ...
            try {
                ...
                User usuario = User.find("username=? and password=?", username, password).first();
                if (usuario!=null){
                    if (usuario.getStatus() == Constants.USER_STATES_BLOCKED){
                        return "blocked";
                    }
                    return "true";
                }
